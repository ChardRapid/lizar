...
public class Http {
	...
	public static String getParameter(HttpServletRequest request,String name,String encode_type,boolean need_trans_code,String defaultValue) throws UnsupportedEncodingException{
		String value="";
		if(need_trans_code){
			try{
			value= Http.escape(new String(request.getParameter(name).getBytes("ISO-8859-1"),encode_type));
			}catch(Exception e){
				return defaultValue;
			}
		}else{
		...
		if(StringHelper.isNull(value))value=defaultValue;
		return value;
	}
	
	public static String url_internal_indicator_filter(String url){
		if(StringHelper.isNull(url))return url;
		int pos=url.indexOf("#");
		if(pos==-1)return url;
	...
	public static String getParameter(HttpServletRequest request,String name,String encode_type,String defaultValue){
	...
	public static String encode_url_chinese(String url,String encode_type) throws UnsupportedEncodingException{
		if(StringHelper.isNull(url))return url;
		if(StringHelper.isNull(encode_type))encode_type="UTF-8";
	...
	public static boolean is_local_url(String domain){
		...
		if(domain.startsWith("172.")){
			...
			if(StringHelper.isInteger(range)){
				...
				if(i<32&&i>15)return true;
			}
		}
		if(domain.equals("localhost"))return true;
		if(domain.equals("127.0.0.1"))return true;
		return false;
	}
	...
	public static Object getAttribute(HttpServletRequest request,String name,Object defaultValue){
	...
	public static int getAttribute(HttpServletRequest request,String name,int defaultValue){
		int value=0;
		try{
	...
	public static long getAttribute(HttpServletRequest request,String name,long defaultValue){
		long value=0;
		try{
	...
	public static char getParameter(HttpServletRequest request,String name,char defaultValue){
		char value=0;
		try{
	...
	public static  String getWholeDomain(String url){
		   if(StringHelper.isNull(url))return "";
		   if(!url.startsWith("http://")&&!url.startsWith("https://")&&!url.startsWith("ftp://")&&!url.startsWith("sftp://")) return "";
		   String result=url.substring(url.indexOf("://")+3,url.length() );
		   int rest_point=result.indexOf("/");
		   if(rest_point==-1){
			   if(!StringHelper.is_normal_char(result.charAt(result.length()-1)))return result.substring(0,result.length()-1);
			   return result;
		   }
	...
	public static String check_domain(String domain){
		...
		for(int i=1;i<domain.length()-1;i++){
			...
			if(!((start<='9'&&start>='0') || (start<='z'&& start>='a') || (start<='Z'&&start>='A')||start=='.')){
				return "wrong args";
			}
		}
		return null;
	...
	public static String post(String url,String data,int timeout) throws IOException{
		 URL _url=new URL(url);
		 StringBuilder content=new StringBuilder();
        URLConnection conn=_url.openConnection();  
        conn.setDoOutput(true); 
	   ...
	   private static String getSerialParameters(Map<String,String> parameters, boolean onlySerialValue) throws UnsupportedEncodingException {
	        String str = "";
	        int i = 0;
	        for(Entry<String,String> e:parameters.entrySet()){
	        	if (onlySerialValue) {
	                str += e.getKey() + "=" + URLEncoder.encode(e.getValue(), "UTF-8");
