...
public class MapReduceOutput {
    ...
    public MapReduceOutput( DBCollection from , Entity cmd, CommandResult raw ){
        _commandResult = raw;
        _cmd = cmd;
        ...
        if ( raw.containsField( "results" ) ) {
            _coll = null;
            _collname = null;
            _resultSet = (Iterable<Entity>) raw.get( "results" );
        } else {
            Object res = raw.get("result");
            if (res instanceof String) {
                _collname = (String) res;
            } else {
                MEntity output = (MEntity) res;
                _collname = output._string("collection");
                _dbname = output._string("db");
            }
        ...
        _counts = (MEntity)raw.get( "counts" );
    }
    ...
    String _dbname = null;
    final Iterable<Entity> _resultSet;
    final DBCollection _coll;
    final MEntity _counts;
    final Entity _cmd;
}
