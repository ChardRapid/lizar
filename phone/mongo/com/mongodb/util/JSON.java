...
public class JSON {
    ...
    public static void serialize( Object o , StringBuilder buf ){
	    ...
	    serialize(new MEntity("$date", format.format(d)), buf);
            return;
        }

        if (o instanceof DBRefBase) {
            DBRefBase ref = (DBRefBase)o;
            MEntity temp = new MEntity();
        ...
        if ( o instanceof Pattern ) {
            Entity externalForm = new MEntity();
        ...
        if ( o instanceof MinKey ){
            serialize( new MEntity("$minKey", 1), buf );
            return;
        }
        if ( o instanceof MaxKey ){
            serialize( new MEntity("$maxKey", 1), buf );
            return;
        }
