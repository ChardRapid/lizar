 ...
 * @dochub cursors
 */
public class DBCursor implements Iterator<Entity> , Iterable<Entity> {
     ...
     */
    public DBCursor( DBCollection collection , Entity q , Entity k, ReadPreference preference ){
        _collection = collection;
        _query = q == null ? new MEntity() : q;
        _keysWanted = k;
        if ( _collection != null ){
    ...
    public DBCursor copy() {
        ...
        if ( _specialFields != null )
            c._specialFields = new MEntity( _specialFields.toMap() );
        return c;
    }
     ...
     */
    public DBCursor sort( Entity orderBy ){
        if ( _it != null )
     ...
     */
    public DBCursor hint( Entity indexKeys ){
        if ( _it != null )
    ...
    private void _check()
        ...
        if ( _collection != null && _query != null ){
            ...
            if ( hasSpecialQueryFields() ){
                foo = _specialFields == null ? new MEntity() : _specialFields;
        ...
        if ( _it == null ){
            _it = (new LinkedList<Entity>()).iterator();
            _fake = true;
        }
    }
    ...
    private void _lookForHints(){
        ...
        for ( Entity o : _collection._hintFields ){
    ...
    void _addToQueryObject( Entity query , String field , Entity thing , boolean sendEmpty ){
        if ( thing == null )
            return;
    ...
    void _addToQueryObject( Entity query , String field , Object thing ){
     ...
     * @throws MongoException
     */
    public Entity next() throws MongoException {
        _checkType( CursorType.ITERATOR );
        return _next();
    }
     ...
     * @throws MongoException
     */
    public List<Entity> toArray( int max )
        throws MongoException {
        _checkType( CursorType.ARRAY );
        _fill( max - 1 );
        return _all;
    ...
    private final Entity _query;
    private final Entity _keysWanted;

    private Entity _orderBy = null;
    private String _hint = null;
    private Entity _hintDBObj = null;
    ...
    private Entity _specialFields;

    // ----  result info ----
    private Iterator<Entity> _it = null;
    private boolean _fake = false;

    private CursorType _cursorType = null;
    private Entity _cur = null;
    private int _num = 0;

    private final ArrayList<Entity> _all = new ArrayList<Entity>();
}
